...
NEW: Enterprise Savings Calculator (Cr) & Full 5-Constraint Linear Programming Solver.
"""
...
with tab4:
    ...
    with st.expander("⚙️ Advanced Operational Parameters & Penalty Adjustments", expanded=False):
        ...
        with ec1:
            ...
            st.markdown("*VIU Operational Penalties (vs Briquette)*")
            mc_thermal = st.slider("Net Thermal Pickup ΔT (°C)", 0.0, 5.0, 3.0, key="m_t")
            mc_cycle = st.slider("Cycle Time Saved (mins)", 0.0, 5.0, 2.5, key="m_c")
            mc_rec_diff = st.slider("Recovery Buffer Δ (%)", 0.0, 3.0, 1.5, key="m_r")
            mc_reblow = st.slider("Reblow Risk Δ (%)", 0.0, 3.0, 1.0, key="m_rb")
            mc_clean = st.slider("Cleanliness Penalty (₹/MT)", 0, 3000, 1200, key="m_cl", step=100)
            mc_yield = st.slider("Yield Loss (%)", 0.000, 0.100, 0.030, format="%.3f", key="m_y")
            mc_carbon_pen = st.slider("Carbon Correction (₹/MT)", 0, 1000, 263, key="m_cp", step=10)
            
            mc_penalty = (mc_thermal * 485) + (mc_cycle * 1150) + (mc_rec_diff * 1220) + (mc_reblow * 158) + (mc_yield * 60000) + mc_clean + mc_carbon_pen
        ...
        with ec2:
            ...
            lc_carbon = st.number_input("LC Carbon %", value=0.10, step=0.01)
            
            st.markdown("*VIU Operational Penalties (vs Briquette)*")
            lc_thermal = st.slider("Net Thermal Pickup ΔT (°C)", 0.0, 5.0, 1.5, key="l_t")
            lc_cycle = st.slider("Cycle Time Saved (mins)", 0.0, 5.0, 1.0, key="l_c")
            lc_rec_diff = st.slider("Recovery Buffer Δ (%)", 0.0, 3.0, 0.5, key="l_r")
            lc_reblow = st.slider("Reblow Risk Δ (%)", 0.0, 3.0, 1.0, key="l_rb")
            lc_clean = st.slider("Cleanliness Penalty (₹/MT)", 0, 3000, 480, key="l_cl", step=100)
            lc_yield = st.slider("Yield Loss (%)", 0.000, 0.100, 0.015, format="%.3f", key="l_y")
            lc_carbon_pen = st.slider("Carbon Correction (₹/MT)", 0, 1000, 265, key="l_cp", step=10)
            
            lc_penalty = (lc_thermal * 485) + (lc_cycle * 1150) + (lc_rec_diff * 1020) + (lc_reblow * 158) + (lc_yield * 60000) + lc_clean + lc_carbon_pen
...
with tab5:
    ...
    if len(blend_alloys) != 2:
        ...
        # Defined grades from Solver_Model.csv (Scaled to True Percentages)
        grades = {
            "Commodity Structural (IS2062/E250)": {"c_lim": 1.00, "reblow_lim": 5.0, "rec_lim": 6.0, "cln_lim": 0.12, "rh_lim": 2.5},
            "TMT/Rebar (Fe500D)":                 {"c_lim": 0.80, "reblow_lim": 4.0, "rec_lim": 5.0, "cln_lim": 0.10, "rh_lim": 2.0},
            "HSLA (API X60/X70)":                 {"c_lim": 0.50, "reblow_lim": 3.0, "rec_lim": 3.0, "cln_lim": 0.07, "rh_lim": 1.5},
            "Automotive Structural (DP600/780)":  {"c_lim": 0.35, "reblow_lim": 2.0, "rec_lim": 2.0, "cln_lim": 0.05, "rh_lim": 1.0},
            "Electrical Steel (CRGO/CRNO)":       {"c_lim": 0.20, "reblow_lim": 1.5, "rec_lim": 1.5, "cln_lim": 0.025, "rh_lim": 0.5},
            "IF Steel (Deep Draw IF)":            {"c_lim": 0.15, "reblow_lim": 1.0, "rec_lim": 1.0, "cln_lim": 0.03, "rh_lim": 0.5}
        }
        ...
        max_r = sc2.slider("Max Reblow Risk (%)", 0.5, 6.0, grades[sel_grade]["reblow_lim"], step=0.5)
        max_rec = sc3.slider("Recovery Stability (±%)", 0.5, 6.0, grades[sel_grade]["rec_lim"], step=0.5)
        max_cln = sc4.slider("Cleanliness Limit", 0.01, 0.20, grades[sel_grade]["cln_lim"], step=0.01)
        max_rh  = sc5.slider("Max RH Time (min)", 0.0, 3.0, grades[sel_grade]["rh_lim"], step=0.1)
        ...
        # Mapping dictionaries for the solver constraints
        ...
        b_ub = [max_c, max_r, max_rec, max_cln, max_rh]
        ...
        if res.success:
            ...
            valid_singles = []
            if carb_map[a1] <= max_c and reblow_map[a1] <= max_r and rec_map[a1] <= max_rec and cln_map[a1] <= max_cln and rh_map[a1] <= max_rh: 
                valid_singles.append(tco_map[a1])
            if carb_map[a2] <= max_c and reblow_map[a2] <= max_r and rec_map[a2] <= max_rec and cln_map[a2] <= max_cln and rh_map[a2] <= max_rh: 
            ...
            pie_colors = {"MC FeMn": "#FF9800", "LC FeMn": "#2196F3", "Mn Briquette": "#4CAF50"}
            fig_pie = go.Figure(data=[go.Pie(labels=[f"{a1} Share", f"{a2} Share"], values=mix, hole=0.4, marker_colors=[pie_colors[a1], pie_colors[a2]])])
            fig_pie.update_layout(title=f"Optimal Procurement Ratio for {sel_grade}", height=380, template="plotly_white")
            st.plotly_chart(fig_pie, use_container_width=True)
            
        else:
            st.error("⚠️ **Constraint Violation:** The chosen metallurgical limits are too strict to be met using these two alloys. Please relax the constraints or switch to a higher-purity alloy blend.")
