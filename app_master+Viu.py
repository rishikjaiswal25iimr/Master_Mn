...
# Physics & Valuation Constants (From Solver Base)
HEAT_SIZE = 190.0
POWER_RATE = 6.5
...
def compute_operational_penalty(thermal, cycle, rec_diff, reblow, clean, yield_loss, rh, eff_mn, price):
    power = (thermal * 0.75 * HEAT_SIZE) / 3.6 / 0.78 * POWER_RATE
    electrode = (thermal * 0.75 * HEAT_SIZE) / 3.6 / 0.78 * 0.0028 * ELECTRODE_RATE
    throughput = (cycle / 53.0) * HEAT_SIZE * 0.30 * THROUGHPUT_MARGIN
    rec_ben = ((rec_diff / 100.0) / eff_mn) * price * 0.50
    reblow_ben = (reblow / 100.0) * REBLOW_COST * (200.0 / HEAT_SIZE)
    clean_ben = STEEL_VALUE * (clean / 100.0) * 200.0 * 0.40
    yield_ben = STEEL_VALUE * (yield_loss / 100.0) * 200.0 * 0.50
    rh_ben = (rh / 100.0) * RH_COST * (200.0 / HEAT_SIZE)
...
with tab4:
    ...
    with st.expander("⚙️ Advanced Operational Parameters & Penalty Adjustments", expanded=False):
        ...
        with ec1:
            ...
            mc_thermal = st.slider("Net Thermal Pickup ΔT (°C)", 0.0, 5.0, 2.5, key="m_t")
            mc_cycle = st.slider("Cycle Time Saved (mins)", 0.0, 5.0, 2.0, key="m_c")
            mc_rec_diff = st.slider("Recovery Buffer Δ (%)", 0.0, 5.0, 2.0, key="m_r")
            mc_reblow = st.slider("Reblow Risk Δ (%)", 0.0, 5.0, 2.0, key="m_rb")
            mc_clean = st.slider("Cleanliness Rejection Δ (%)", 0.00, 0.10, 0.05, format="%.3f", key="m_cl")
            mc_yield = st.slider("Yield Loss Δ (%)", 0.00, 0.10, 0.03, format="%.3f", key="m_y")
            mc_rh = st.slider("RH / Carbon Correction Δ (%)", 0.0, 5.0, 2.0, key="m_rh")
        ...
        with ec2:
            ...
            lc_rec_diff = st.slider("Recovery Buffer Δ (%)", 0.0, 5.0, 0.5, key="l_r")
            lc_reblow = st.slider("Reblow Risk Δ (%)", 0.0, 5.0, 1.0, key="l_rb")
            ...
            lc_rh = st.slider("RH / Carbon Correction Δ (%)", 0.0, 5.0, 1.0, key="l_rh")
