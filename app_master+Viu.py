...
NEW: Granular VIU component sliders, dynamic alloy selection, and a 2-Alloy LP Solver.
"""
...
with tab4:
    ...
    with st.expander("⚙️ Advanced Operational Parameters & Penalty Adjustments", expanded=False):
        ...
        with ec1:
            ...
            # Hidden computation logic converting physical parameters into ₹ penalties
    ...
    # ── Recommendation Engine (Filtered by Active Alloys) ──
    if active_alloys:
...
with tab5:
    ...
    if len(blend_alloys) != 2:
        ...
        # Mapping dictionaries for the solver
        ...
        if res.success:
            ...
            # Baseline calculation if they just used the cheapest single valid alloy
            valid_singles = []
            if carb_map[a1] <= max_c and reblow_map[a1] <= max_r: valid_singles.append(tco_map[a1])
            if carb_map[a2] <= max_c and reblow_map[a2] <= max_r: valid_singles.append(tco_map[a2])
            ...
            st.error("⚠️ **Constraint Violation:** The chosen Carbon or Reblow limits are too strict to be met using these two alloys. Please relax the constraints or switch to a higher-purity alloy blend.")